...
const IdempotencyKeys = {
  ...
  smeClarification: (updateId) => `SME_CLARIFICATION_AVAILABLE:${updateId}`,
  yaEvidenceCorrection: (reportId) => `YA_EVIDENCE_CORRECTION:${reportId}`,
}
...
async function sendSmeClarificationAvailableEmail(update = {}, request = {}, opts = {}) {
  const fields = requestEmailFields(request)
  const to = fields.smeEmail || sliceStr(update.smeEmail || '', 200)
  if (!to) return { sent: false, skipped: true, error: 'No SME email' }
  return sendViaResend({
    to,
    templateId: 'sme_clarification_available',
    input: {
      ...fields,
      title: update.title,
      content: update.content,
      updateType: update.updateType,
      requestId: update.briefingRequestId || fields.requestId,
      tenderTitle: fields.tenderTitle || update.tenderTitle,
      tenderNumber: fields.tenderNumber || update.tenderNumber,
    },
    idempotencyKey: IdempotencyKeys.smeClarification(update.id),
    env: opts.env,
    resendClient: opts.resendClient,
    deps: opts.deps,
    meta: {
      eventType: 'sme_clarification_available',
      entityId: update.id,
      recipientUserId: update.smeId || fields.smeId,
    },
  })
}

async function sendYaEvidenceCorrectionEmail(request = {}, agent = {}, detail = '', opts = {}) {
  const fields = requestEmailFields(request)
  const agentEmail = sliceStr(agent.email || '', 200)
  const agentId = sliceStr(agent.id || agent.uid || fields.agentId, 128)
  if (!agentEmail) return { sent: false, skipped: true, error: 'No agent email' }
  const reportId = opts.reportId || fields.requestId
  return sendViaResend({
    to: agentEmail,
    templateId: 'ya_evidence_correction',
    input: {
      ...fields,
      detail,
      requestId: fields.requestId,
    },
    idempotencyKey: IdempotencyKeys.yaEvidenceCorrection(reportId),
    env: opts.env,
    resendClient: opts.resendClient,
    deps: opts.deps,
    meta: {
      eventType: 'ya_evidence_correction',
      entityId: fields.requestId,
      recipientUserId: agentId,
    },
  })
...
const sendSmeClarificationAvailableEmailSafe = (update, request, opts) =>
  safe(sendSmeClarificationAvailableEmail, update, request, opts)
const sendYaEvidenceCorrectionEmailSafe = (request, agent, detail, opts) =>
  safe(sendYaEvidenceCorrectionEmail, request, agent, detail, opts)
...
module.exports = {
  ...
  sendSmeClarificationAvailableEmail,
  sendYaEvidenceCorrectionEmail,
  ...
  sendSmeClarificationAvailableEmailSafe,
  sendYaEvidenceCorrectionEmailSafe,
