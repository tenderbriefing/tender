...
export default function FounderAgentDetailPage() {
  ...
  return (
    <FounderV2Gate>
      <FounderShell
        title={String(user.displayName || user.name || 'Youth Agent')}
        subtitle="Profile and assignment history"
      >
        {loading && !detail ? (
          <LoadingState />
        ) : error ? (
          ...
          <div className="space-y-8">
            <dl className="grid gap-5 rounded-md border border-slate-200 bg-white p-5 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <Field
                ...
                value={
                  user.completedBriefingCount != null ? String(user.completedBriefingCount) : undefined
                }
              />
            </dl>

            <section>
              <h2 className="text-sm font-semibold text-brand-900">Assignments</h2>
              {requests.length === 0 ? (
                <p className="mt-3 text-sm text-slate-500">No assignments in the recent cohort.</p>
              ) : (
                <ul className="mt-3 divide-y divide-slate-100 overflow-hidden rounded-md border border-slate-200 bg-white">
                  {requests.map((r) => (
                    <li key={String(r.id)} className="flex items-center justify-between gap-4 px-4 py-3">
                      ...
                      <span className="text-xs text-slate-500">{String(r.status || '')}</span>
                    </li>
                  ))}
                </ul>
              )}
