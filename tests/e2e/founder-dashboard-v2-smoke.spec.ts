...
test.describe('Founder Dashboard V2 signed-in walkthrough', () => {
  test('Overview KPIs, periods, chart, Needs Attention', async ({ page }) => {
    ...
    await expect(page.getByText('Paid Bookings', { exact: true })).toBeVisible({ timeout: 30_000 })
    ...
    await expect(page.locator('dt', { hasText: 'SMEs' }).first()).toBeVisible()
    await expect(page.locator('dt', { hasText: 'Youth Agents' }).first()).toBeVisible()
  ...
  test('SME, Youth Agent, Briefings, Settings links', async ({ page }) => {
    ...
    await expect(page.locator('thead th', { hasText: 'Company' })).toBeVisible()
    ...
    await expect(page.locator('thead th', { hasText: 'Completed' })).toBeVisible()
  ...
  test('responsive Overview at 375 and 1280', async ({ page }) => {
    ...
    for (const width of [375, 1280]) {
      ...
      await expect(page.getByRole('heading', { name: 'Overview' })).toBeVisible({ timeout: 30_000 })
      await expect(page.getByText('Revenue', { exact: true })).toBeVisible({ timeout: 30_000 })
